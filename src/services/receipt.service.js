...
const addReceipt = async (dataRequest) => {
	const request = new RequestModel(`${env.TRANSACTIONAL_API_URL}/receipts`, new FormData());
	request.deleteHeader("Content-Type");
	request.body.append("referralGuide", dataRequest.file);
	request.body.append("dataJsonContent", JSON.stringify(dataRequest.jsonContent));
